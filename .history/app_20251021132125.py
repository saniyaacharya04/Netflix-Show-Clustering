...
# ----------------------------
# Genre Distribution Heatmap (Fixed)
# ----------------------------
st.subheader("Genre Distribution per Cluster")
if not filtered_genres_dummies.empty:
    genre_cols = filtered_genres_dummies.columns.tolist()
    genre_cluster = pd.concat([filtered_df['Cluster'], filtered_genres_dummies], axis=1).groupby('Cluster')[genre_cols].sum()
    if not genre_cluster.empty:
        fig, ax = plt.subplots(figsize=(12,6))
        sns.heatmap(genre_cluster, annot=True, fmt="d", cmap="YlGnBu", ax=ax)
        st.pyplot(fig)
    else:
        st.write("No genre distribution available for current filters.")
else:

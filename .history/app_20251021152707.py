...
if display_mode == "Single Clustering":
    ...
    st.pyplot(genre_heatmap(filtered_df, genres_dummies), key="single_heatmap")

    st.subheader("Rating Distribution Boxplot")
    st.pyplot(rating_boxplot(filtered_df), key="single_boxplot")
    ...
    for c in sorted(filtered_df['Cluster'].unique()):
        ...
        if plt_wc:
            st.pyplot(plt_wc, key=f"single_wc_{c}")
...
# -----------------------------
    ...
    st.plotly_chart(plots['Hierarchical'], key="comp_hierarchical")

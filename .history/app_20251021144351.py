...
@st.cache_data
def load_data():
    df = pd.read_csv('data/netflix_titles.csv')  # Make sure the CSV is in the same folder as app.py
...
if not filtered_genres_dummies.empty:
    # Only use genres that exist in the filtered data
    genre_cols = [col for col in filtered_genres_dummies.columns if col in filtered_genres_dummies.columns]
    genre_cluster = filtered_df.groupby('Cluster')[genre_cols].sum()

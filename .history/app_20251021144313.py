# ===============================================
# Ultimate Netflix Clustering Dashboard (Fixed)
# ===============================================
import pandas as pd
import numpy as np
...
# Sidebar Controls
# ----------------------------
st.title("🎬 Netflix Clustering Dashboard (Fixed)")
cluster_method = st.sidebar.selectbox("Clustering Method", ["K-Means", "Hierarchical"])
n_clusters = st.sidebar.slider("Number of Clusters", 2, 10, 5)

# Filters
st.sidebar.subheader("Filter Shows")
selected_genres = st.sidebar.multiselect("Genres", options=sorted({g for lst in df['genres'] for g in lst}))
...
# ----------------------------
# Genre Distribution Heatmap (Safe)
# ----------------------------
st.subheader("🔥 Genre Distribution per Cluster")
if not filtered_genres_dummies.empty:
    genre_cluster = filtered_df.groupby('Cluster')[filtered_genres_dummies.columns].sum()
    fig, ax = plt.subplots(figsize=(12,6))
    sns.heatmap(genre_cluster, annot=True, fmt="d", cmap="YlGnBu", ax=ax)
    st.pyplot(fig)
else:
    st.write("No genre data available for selected filters.")
...
# ----------------------------
st.subheader("⚔️ Clustering Comparison: K-Means vs Hierarchical")

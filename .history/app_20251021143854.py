...
# Genre Heatmap
# ----------------------------
existing_genres = [g for g in mlb.classes_ if g in genre_dummies.columns]
if existing_genres:
    st.subheader("Genre Distribution per Cluster")
    heatmap_fig = plot_genre_heatmap_actual(filtered_df, genre_cols=existing_genres)
    st.pyplot(heatmap_fig)
else:
    st.warning("No valid genre columns found for heatmap.")
...
for c in sorted(filtered_df['Cluster'].unique()):
    ...
    text = " ".join(filtered_df[filtered_df['Cluster']==c]['description'].dropna().tolist())
    if text.strip():
...
for i in range(n_clusters):
    ...
    if not cluster_data.empty:
        ...
        st.write(cluster_data[['title','type','rating','duration','listed_in']].head(5))
    else:
        st.write(f"Cluster {i} has no shows.")

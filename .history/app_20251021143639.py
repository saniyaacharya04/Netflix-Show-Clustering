...
st.pyplot(plot_genre_heatmap_actual(filtered_df, genre_cols=mlb.classes_))
...
selected_clusters = st.multiselect(
    ...
    default=sorted(filtered_df['Cluster'].unique())
)
if selected_clusters:
    filtered_cluster_df = filtered_df[filtered_df['Cluster'].isin(selected_clusters)]
else:
    filtered_cluster_df = filtered_df.copy()

st.markdown(f"Showing {len(filtered_cluster_df)} shows after cluster filter.")

# ----------------------------
# Download Filtered Clustered Data (updated)
# ----------------------------
st.subheader("Download Filtered & Clustered Data")
csv_data = filtered_cluster_df.to_csv(index=False).encode('utf-8')
st.download_button(
    label="Download CSV",
    data=csv_data,
